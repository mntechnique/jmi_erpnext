...
def _execute(filters, additional_table_columns=None, additional_query_columns=None):
	...
	for inv in invoice_list:
		...
		row = [
			 inv.customer, inv.name, inv.posting_date, inv.po_no , "unused", "unused" , "unused" , "unused", inv.owner
		]
		...
		row +=[
			...
			inv.debit_to, inv.get("tax_id"), 10 , 10, 2, 10 , "Description	" , 40200 , "Rate of item" , 10 , "Amount" , 10 , "Unused" ,
			2 , 100 , "Serial Number" , inv.get("tax_id")
			# ", ".join(delivery_note),", ".join(cost_center),
...
def get_columns(invoice_list, additional_table_columns):
	...
	columns +=[
		...
		_("Sales Representative ID") + "::120",	  
		 _("Receivable Account") + ":Link/Account:120", _("Sales Tax Id") + "::80",
		 _("Number of Distributions") + "::120",  _("Invoice/CM Distribution") + "::120" ,
		 _("Quantity") + "::100", _("Item ID") + "::100", _("Description") + "::100",
		 _("G/L Account") + "::100" , _("Unit Price") + "::100" , _("Tax Type") + "::100" ,
		 _("Amount") + "::80" , _("U/M ID") + "::80" , _("U/M No of Stocking units") + "::180",
		 _("Stocking Quantity") + "::120" , _("Stocking Unit Price") + "::150" ,
